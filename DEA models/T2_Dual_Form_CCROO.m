...
m=size(X',1); 
s=size(Y',1); 
epsilon=1E-9;   % Epsilon Value being changed
% CCR_OO
...
W(i,:)=w;
E(i, i)=X(i,:)*W(i,1:m)';    % Get Efficiency(Eii) for DMUi
end  
W    % the best weight for all DMUs
Omega=W(:,1:m)    % Input Weights
mu=W(:,m+1:m+s)   % Output Weights

%% Generate the output file "DEA_T2_OO_Output_Data_File.table";
...
DEA_score_CCROO=diag(E)
table_names=["DMUs","Input_1","Input_2","Output_1","Output_2", "Weight_v1",...
    "Weight_v2","Weight_u1","Weight_u2","DEA_Score"]
DEA_T2_OO_Output_Data_File_table=table(nameDMU,X(:,1),X(:,2),Y(:,1),Y(:,2),...
    W(:,1),W(:,2),W(:,3),W(:,4),DEA_score_CCROO)
DEA_T2_OO_Output_Data_File_table.Properties.VariableNames = {'DMUs','Input_1','Input_2','Output_1','Output_2', 'Weight_v1',...
    'Weight_v2','Weight_u1','Weight_u2','DEA_Score'}
filedata ='DEA_T2_OO_Output_Data.xlsx'
writetable(DEA_T2_OO_Output_Data_File_table,filedata)
...
filename = 'DEA_T2_OO_Statistics_Weights.xlsx';
